...
<head>
    ...
    <title>Idir Management - Equb & Idir</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .note { font-size: 14px; color: #666; margin-top: 8px; font-style: italic; }
    </style>
</head>
<body>
...
<%
    ...
    request.setAttribute("activePage", "idir");
    ...
    // Available members: only those NOT in any Idir group
    ...
    if (idirMembers == null) idirMembers = java.util.Collections.emptyList();
%>
...
<div class="main-content" id="mainContent">
    <div class="welcome-header">
        <h1><i class="fas fa-heart"></i> <%= isAm ? "የእድር አስተዳደር" : "Idir Management" %></h1>
    ...
    <div class="grid">
        <!-- Create Idir Group -->
        <div class="card">
            <h2><i class="fas fa-plus-circle"></i> <%= isAm ? "አዲስ የእድር ቡድን ፍጠር" : "Create Idir Group" %></h2>
            <form method="post" action="<%= ctx %>/admin/idir">
                <input type="hidden" name="action" value="create_group" />

                <div class="form-group">
                    <label><%= isAm ? "የቡድን ስም" : "Idir Name" %></label>
                    <input type="text" name="idir_name" required />
                </div>

                <div class="form-group">
                    <label><%= isAm ? "ወርሃዊ ክፍያ (ብር)" : "Monthly Payment (ETB)" %></label>
        ...
        <!-- List of Groups -->
        <div class="card">
            <h2><i class="fas fa-list"></i> <%= isAm ? "የእድር ቡድኖች" : "Idir Groups" %></h2>

            <% if (groups.isEmpty()) { %>
            <div style="text-align:center;padding:40px;color:#888;">
                <i class="fas fa-folder-open" style="font-size:48px;color:#c9a227;margin-bottom:15px;"></i>
                <p><%= isAm ? "ምንም የእድር ቡድን የለም።" : "No Idir groups created yet." %></p>
            </div>
            <% } else { %>
            <div class="table-container">
                <table>
                    ...
                    <tr>
                        <th>ID</th>
                        <th><%= isAm ? "ስም" : "Name" %></th>
                        <th><%= isAm ? "ወርሃዊ ክፍያ" : "Monthly Payment" %></th>
                        <th><%= isAm ? "ክፈት" : "Open" %></th>
                    ...
                    <tr>
                        <td><%= g.getIdirId() %></td>
                        <td><%= g.getIdirName() %></td>
                        <td><%= String.format("%,.2f", g.getMonthlyPayment()) %> ETB</td>
                        <td>
                            <a href="<%= ctx %>/admin/idir?idir_id=<%= g.getIdirId() %>" style="color:#1e4d2b;font-weight:600;">
                                <i class="fas fa-folder-open"></i> <%= isAm ? "ክፈት" : "Open" %>
                            </a>
                        </td>
                        <td class="actions">
                            <form method="post" action="<%= ctx %>/admin/idir" style="display:inline;">
    ...
    <div class="card">
        <h2><i class="fas fa-users"></i> <%= isAm ? "የቡድን አባላት (እድር ID: " : "Group Members (Idir ID: " %><%= selectedIdirId %>)</h2>

        <!-- Add Member -->
        <div style="margin-bottom:30px;">
            <h3 style="color:#1e4d2b;margin-bottom:16px;"><%= isAm ? "አባል ጨምር" : "Add Member to this Idir" %></h3>
            <form method="post" action="<%= ctx %>/admin/idir">
                <input type="hidden" name="action" value="add_member" />
                <input type="hidden" name="idir_id" value="<%= selectedIdirId %>" />

                <div class="form-group">
                    <label><%= isAm ? "አባል ምረጥ (ከፍተኛ 1 ቡድን)" : "Select Member (Max 1 group)" %></label>
                    ...
                    <div class="note">
                        <%= isAm ? "አባላት በአንድ የእድር ቡድን ብቻ መሆን ይችላሉ።"
                                : "Members can only be in one Idir group." %>
                    </div>
                </div>
